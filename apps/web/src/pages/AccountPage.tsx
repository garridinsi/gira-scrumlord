...
function ReadOnlyRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, padding: '8px 0', borderBottom: '1px dashed var(--eg-rule)' }}>
...
export function AccountPage() {
  ...
  if (me.isLoading) return <div className="mono" style={{ padding: 32 }}>cargando · loading…</div>;
  ...
  return (
    <div style={{ width: '100%', maxWidth: 720, margin: '0 auto', padding: '32px 24px 64px', boxSizing: 'border-box' }}>
      ...
      <Card title="perfil · profile">
        ...
        <input id="acct-name" style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} maxLength={120} />

        <label className="caps" htmlFor="acct-locale" style={{ ...labelStyle, marginTop: 16 }}>
          idioma · language
        </label>
        <select id="acct-locale" style={inputStyle} value={locale} onChange={(e) => setLocale(e.target.value as UserLocale)}>
          {LOCALES.map((l) => (
      ...
      <Card title="identidad · identity" sub="El correo es tu identidad de acceso · email is your login identity">
        <ReadOnlyRow label="correo · email" value={u.email} />
        <ReadOnlyRow label="rol · role" value={ROLE_LABEL[u.role] ?? u.role} />
        <ReadOnlyRow label="tipo · kind" value={u.kind === 'client' ? 'Cliente · Client' : 'Equipo · Staff'} />
        ...
        <div style={{ marginTop: 20, paddingTop: 16, borderTop: '2px solid var(--eg-iron)' }}>
          ...
          <p className="mono" style={{ fontSize: 11, color: 'var(--eg-fg-4)', marginBottom: 10, lineHeight: 1.5 }}>
            Enviaremos un enlace al NUEVO correo. El cambio solo se aplica al confirmarlo · we email the
            NEW address; the switch happens only when you confirm it.
          </p>
          ...
          {requestEmail.isSuccess && (
            <div className="mono" style={{ fontSize: 12, color: 'var(--eg-green, #2a7)', marginTop: 10 }}>
      ...
      <Card title="sesiones activas · active sessions" sub="Dispositivos con sesión abierta · devices currently signed in">
        {sessionsQ.isLoading && <div className="mono" style={{ fontSize: 12 }}>cargando…</div>}
        {(sessionsQ.data ?? []).map((s) => (
          <div
            key={s.id}
            style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '10px 0', borderBottom: '1px dashed var(--eg-rule)' }}
          >
            <div style={{ minWidth: 0 }}>
              <div className="mono" style={{ fontSize: 12, color: 'var(--eg-iron)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {s.userAgent ?? 'dispositivo desconocido · unknown device'}
              </div>
              <div className="mono" style={{ fontSize: 10, color: 'var(--eg-fg-4)' }}>
                {s.ip ?? '—'} · {s.lastSeenAt ? `visto ${new Date(s.lastSeenAt).toLocaleString('es-ES')}` : `desde ${new Date(s.createdAt).toLocaleDateString('es-ES')}`}
              </div>
            </div>
            {s.current && (
              <span className="caps" style={{ color: 'var(--eg-iron)', background: 'var(--eg-yellow)', padding: '2px 8px', height: 'fit-content' }}>
                actual · current
              </span>
            )}
          </div>
        ...
        {revokeOthers.isSuccess && (
          <span className="mono" style={{ fontSize: 12, color: 'var(--eg-green, #2a7)', marginLeft: 12 }}>
