...
describe('clients + projects + isolation', () => {
  ...
  it('blocks switching to monthly cadence while a sprint is active; allows it otherwise', async () => {
    ...
    await app.inject({
      method: 'POST',
      url: `/sprints/${sprint.json().id}/close`,
      headers: { cookie },
    });
    const ok = await app.inject({
      method: 'PATCH',
  ...
  it('refuses to delete a client that still has projects (FK RESTRICT, clean 409)', async () => {
    ...
    const client = await prisma.client.create({
      data: { name: 'Has Projects', slug: 'hasprojects' },
    });
    ...
    expect((await prisma.project.findUnique({ where: { key: 'OWNED' } }))?.clientId).toBe(
      client.id,
    );
  });
  ...
  it('refuses to delete a client that still has billing annexes (financial records kept)', async () => {
    const admin = await actingAs({ role: 'admin' });
    const client = await prisma.client.create({
      data: { name: 'Has Annex', slug: 'hasannex', currency: 'EUR' },
    });
  ...
  it('reads + patches a project, lists labels, and patches a status', async () => {
    ...
    await app.inject({
      ...
      url: '/projects',
      headers: { cookie },
      payload: { key: 'PCH', name: 'Patchable' },
    });
    ...
    const st = (
      await app.inject({
        method: 'POST',
        url: '/projects/PCH/statuses',
        headers: { cookie },
        payload: { name: 'QA', category: 'in_progress' },
      })
    ).json();
    const stp = await app.inject({
      method: 'PATCH',
      url: `/statuses/${st.id}`,
      headers: { cookie },
      payload: { name: 'QA Review' },
    });
  ...
  it('records an audit entry when deleting a status or label (parity with create)', async () => {
    ...
    await app.inject({
      ...
      url: '/projects',
      headers: { cookie },
      payload: { key: 'AUD', name: 'Audited' },
    });
    const label = (
      await app.inject({
        method: 'POST',
        url: '/projects/AUD/labels',
        headers: { cookie },
        payload: { name: 'temp' },
      })
    ).json();
    const extraStatus = (
      await app.inject({
        method: 'POST',
        url: '/projects/AUD/statuses',
        headers: { cookie },
        payload: { name: 'Extra', category: 'todo' },
      })
    ).json();

    expect(
      (await app.inject({ method: 'DELETE', url: `/labels/${label.id}`, headers: { cookie } }))
        .statusCode,
    ).toBe(204);
    expect(
      (
    ...
    expect(
      (
        await app.inject({
          method: 'DELETE',
          url: '/labels/cl00000000000000000000000',
          headers: { cookie },
        })
      ).statusCode,
    ).toBe(404);
  ...
  it('refuses to delete a status that still has issues (FK → in_use 409)', async () => {
    ...
    await app.inject({
      ...
      url: '/projects',
      headers: { cookie },
      payload: { key: 'FKP', name: 'FK Project' },
    });
    const statuses = (
      await app.inject({ method: 'GET', url: '/projects/FKP/statuses', headers: { cookie } })
    ).json();
    ...
    await app.inject({
      ...
      url: '/issues',
      headers: { cookie },
      payload: { projectKey: 'FKP', title: 'lives in Backlog' },
    });

    const del = await app.inject({
      method: 'DELETE',
      url: `/statuses/${backlog.id}`,
      headers: { cookie },
    });
