...
describe('escalation', () => {
  beforeEach(async () => {
    ...
    await prisma.notificationChannel.create({
      data: { name: 'oncall', kind: 'email', target: 'oncall@example.test', scope: 'global', events: ['issue.emergency'] },
    });
  });
  ...
  it('re-pages an open incident past the interval and bumps the level', async () => {
    ...
    await prisma.incident.create({
      data: { issueId: issue.id, status: 'open', escalationLevel: 0, lastNotifiedAt: new Date(Date.now() - 10 * 60_000) },
    });
  ...
  it('skips acknowledged incidents', async () => {
    const { issue } = await makeIssue('T-1', 'T');
    await prisma.incident.create({
      data: { issueId: issue.id, status: 'acked', lastNotifiedAt: new Date(Date.now() - 60 * 60_000) },
    });
  ...
  it('does not escalate beyond the max level', async () => {
    const { issue } = await makeIssue('T-1', 'T');
    await prisma.incident.create({
      data: { issueId: issue.id, status: 'open', escalationLevel: 3, lastNotifiedAt: new Date(Date.now() - 60 * 60_000) },
    });
